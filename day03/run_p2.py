...
def test_find_shortest_step_example1():
    wirepaths = [create_path("R75,D30,R83,U83,L12,D49,R71,U7,L72"),create_path("U62,R66,U55,R34,D71,R55,D58,R83")]
    crossings = find_crossings(wirepaths)
    assert find_shortest_step(wirepaths, crossings) == 610

def test_find_shortest_step_example2():
    wirepaths = [create_path("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51"),create_path("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")]
    crossings = find_crossings(wirepaths)
    assert find_shortest_step(wirepaths, crossings) == 410
...
def find_shortest_step(wirepaths, crossings):
    return min([sum([wire.index(crossing)+1 for wire in wirepaths]) for crossing in crossings])


if __name__ == "__main__":
    wirepaths = read_wires("input.txt")
    crossings = find_crossings(wirepaths)
    fastest_crossing = find_shortest_step(wirepaths, crossings)
